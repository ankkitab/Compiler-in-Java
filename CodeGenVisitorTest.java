...
public class CodeGenVisitorTest implements ImageResources{
	...
	public void imageTest1() throws Exception{
		...
		BufferedImage refImage0 = ImageSupport.readImage(imageFile1, 128, 128);
		BufferedImage loggedImage0 = RuntimeLog.globalImageLog.get(0);
		BufferedImage newImage = RuntimeLog.globalImageLog.get(1);
		assertTrue(ImageSupport.compareImages(newImage,loggedImage0));
		keepFrame();
	}
	...
	public void image7() throws Exception {
		...
		for(int y = 0; y < 1024; y++) {
			for (int x = 0; x < 1024; x++) {
				...
				int pixelRef = ImageSupport.getPixel(loggedImage0, RuntimeFunctions.cart_x(r, a),RuntimeFunctions.cart_y(r, a));
...
@Test
public void imag1() throws Exception{
	devel = false;
	grade = true;
	String prog = "image5";
	String input = prog 
...
@Test
public void imageFail1() throws Exception{
	devel = false;
	grade = true;
	String prog = "imageGenMagenta";
	String input = "imageGenMagenta"
			+ "\nimage[512,256] g; "
			+ "\ng[[x,y]] = 255 + 16711680;"
			+ "g -> SCREEN;\n";
...
//	BufferedImage loggedImage1 = RuntimeLog.globalImageLog.get(1);
	...
	for(int y = 0; y < 256; y++) {
		for (int x = 0; x < 512; x++) {
			...
			int pixelRefH = 255+16711680;
            int pixelH = ImageSupport.getPixel(loggedImage0, x,y);
            
            assertEquals(pixelRefH, pixelH);
            //assertEquals(pixelRefG, pixelG);
		}
	}
	keepFrame();
